...
int Engine2::add_object(float x, float y, const RobotPosition & observed_position, int objType) {
   ...
   for (it = m_objects->begin(); it != m_objects->end(); it++) {
      ...
      float x_diff = (*it).x_position - x;
      float y_diff = (*it).y_position - y;
...
//       msg << "add_object() : (" << count++ << "): Distance = " << distance;
      ...
      if ( distance < WSObject::fudge ) {
         std::stringstream ss;
         ss << "add_object(" << x << ", " << y << ") No add - there is an object at (" << (*it).x_position << "," << (*it).y_position << ")." << endl;
   ...
   if (it != m_objects->end()) { // We found the object already
      ...
      std::stringstream ss;
      ss << "Added object at (" << x << ", " << y << ").";
      m_logger->log(ss);
      new_obj = new WSObject(x, y, observation_distance);
      update_object = new_obj;
      count++;
   } 

   // Update R, G, B
   if (objType == CLOSEST_OBJECT) {
...
// }
      ...
      add_objects(current_view, closest_cluster);
   }
   ...
   // TODO closest_cluster
   //
   return prepare_grab(*closest_cluster);
...
bool Engine2::prepare_grab(Point centroid)
{
   ...
   if (!calculate_robot_position(*closest_cluster, new_position))
   {
      m_logger->log("prepare_grab() - the calculated RobotPosition is outside of robot limits! Aborting.");
      return -1;
   } else {
      moveTo(new_position);
      return 0;
...
bool Engine2::add_objects(ClusterCloud & cloud, std::vector<Point>::iterator & closest_cluster)
{
...
// Iterator closest_cluster;
   ...
   for (Iterator i = cloud.begin(); i != cloud.end(); ++i)
   {
      ...
      if (i == closest_cluster) {
         ...
         add_object(pos.x, pos.y, currentPos, CLOSEST_OBJECT);
      } else {
         i->r = 0; i->g = 255; i->b = 0;
         add_object(pos.x, pos.y, currentPos, DEFAULT_OBJECT);
      }
   }
